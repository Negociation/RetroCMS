...
function createDialog(dialogId, header, dialogZIndex, dialogLeft, dialogTop, exitCallback) {
	...
	dialog.style.top = (dialogTop || 0) + "px";
		...
		var exitButton = Builder.node("a", {href:"#", className:"dialog-tv-exit"}, [
			Builder.node("img", {src: habboStaticFilePath + "/images/dialogs/tv-exit.png", width:11, height:11, alt:""})
		]);
		Event.observe(exitButton, "click", exitCallback, false);
		headerBar.push(exitButton);
		}
...
var ClientMessageHandler = {
    ...
    nielsen: function(data){
      ClientMessageHandler.url("http://web.archive.org/web/20071007195943/http://secure-dk.imrworldwide.com/cgi-bin/m?ci=Habbohotel&cg=0&si=" + data);
    },
